...
if (mysqli_num_rows($qrylistofstudentview)) {
	...
	$qry = mysqli_query($connection, "select count(*) as total from departmentreject_announcement_table where isApproved = 'Reject' and stprofID = '" . $stprofID . "' ");
	$res = mysqli_fetch_assoc($qry);
	$whereToStart = $res['total']; //4

	//count if there is next announcement
$qry = mysqli_query($connection, "select * from department_announcement_table where  isApproved = 'Reject' LIMIT " . $whereToStart . ",1 ");
...
if (mysqli_num_rows($qry) > 0) {
	...
	mysqli_query($connection, "insert into departmentreject_announcement_table (stprofID, DannouncementID, departmentClubID, isApproved) values ('" . $stprofID . "', '" . $DannouncementID . "', '".$reslistofstudentview['departmentClubID']."', 'Reject')");
