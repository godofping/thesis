    ...
    <div class="row mt-5">
      <div class="col-md-12">
        ...
        <table class="table" id="dtBasicExample">
          ...
          <tbody>
            <?php 
            $qrycscann = mysqli_query($connection, "select * from council_club_announcement_view where isApproved = 'Yes'");
            while ($rescscann = mysqli_fetch_assoc($qrycscann)) { ?>
               <tr>
              <td scope="row"><?php echo date('F d, Y h:i A', strtotime($rescscann['dateSubmit'])); ?></td>
              <td><?php echo $rescscann['CounName']; ?></td>
        ...
        buttons: [
            {
                ...
                text: 'Save to CSV',
                title:"Departmental Councils posted Announcement"
            },
            {
                extend: 'excel',
                className: 'btn btn-outline btn-sm',
                text: 'Save to Excel',
                ttitle:"Departmental Councils posted Announcement"
            },
            {
                extend: 'print',
                className: 'btn btn-outline btn-sm',
